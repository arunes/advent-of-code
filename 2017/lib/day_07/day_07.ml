...
type program = { id : string; weight : int; children : string list }
...
let input : program list =
  ...
  |> List.map (fun row ->
         ...
         let id_weight_pair =
           id_child_pair |> List.hd |> String.split_on_char ' '
         in
         let id = id_weight_pair |> List.hd in
         let weight =
           id_weight_pair |> List.tl |> List.hd
         ...
         if List.length id_child_pair > 1 then
           let children =
             List.nth id_child_pair 1 |> String.split_on_char ','
             |> List.map (fun c -> c |> String.trim)
           in
           { id; weight; children }
         else (* no child *)
           { id; weight; children = [] })
...
let part1 =
  let rec find_parent id =
    let parent =
      input
      |> List.find_opt (fun p -> p.children |> List.exists (fun c -> c = id))
    in
    match parent with None -> id | Some p -> find_parent p.id
  in
