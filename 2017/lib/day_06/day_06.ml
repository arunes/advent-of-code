...
let find_duplicate is_part_2 = 
  ...
  let rec redistribute cycle remaining current_index current_list completed_lists = 
    ...
    if remaining = 0 && found_dup_index > -1 then
      ...
      | 0 -> (* no more to distribute, add current list to completed lists and get the max again *)
          let max_index = get_max_pos current_list in
          let max_value = List.nth current_list max_index in
          let new_current = current_list |> List.mapi (fun i e -> if i = max_index then 0 else e) in

          redistribute (cycle+1) max_value (get_next_index max_index) new_current (if cycle = 0 then completed_lists else current_list :: completed_lists)
      | _ ->
        (* update the current list *)
        let new_current = current_list |> List.mapi (fun i e -> if i = current_index then e + 1 else e) in
        redistribute cycle (remaining-1) (get_next_index current_index) new_current completed_lists
